...
function formatRelativeTime(dateStr: string, t: (key: string, params?: Record<string, string | number>) => string): string {
  ...
  if (mins < 60) return t("sidebar.timeMinutesAgo", { n: mins });
  if (hours < 24) return t("sidebar.timeHoursAgo", { n: hours });
  if (days < 7) return t("sidebar.timeDaysAgo", { n: days });
  ...
  const inactiveWorktreeSelector = worktreeGuide
    ?? (worktreeLoading && !showWorktreeSwitcher
      ? {
          ...
          title: t("sidebar.worktreesCheckingTitle"),
        }
      : null);
          ...
          return (
            <div ref={wtDropdownRef} style={{ position: "relative", marginTop: 6 }}>
              <button
                ...
                title={currentWt ? t("sidebar.switchWorktreeTo", { path: currentWt.path }) : t("sidebar.switchWorktree")}
                style={{
                  width: "100%",
                  height: 29,
                  boxSizing: "border-box",
      ...
      {confirmDelete ? (
        ...
        <>
          <div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {t("general.delete")} <span style={{ fontWeight: 600 }}>&ldquo;{title.slice(0, 22)}{title.length > 22 ? "…" : ""}&rdquo;</span>{t("sidebar.deleteConfirmSuffix")}
          </div>
        ...
        <>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 2, display: "flex", alignItems: "center", gap: 8, color: "var(--text-dim)", fontSize: 11, minWidth: 0 }}>
              ...
              <span>{t("sidebar.messageCount", { n: session.messageCount })}</span>
              {session.worktreeBranch && (
                <span
                  title={t("sidebar.worktreeTitle", { path: session.cwd })}
