        ...
        <div style={{
          ...
          <div style={{
            ...
            >
              ...
              {copied ? t("general.copied") : t("message.copy")}
            </button>
          </div>
          {(canFork || canNavigate) && (
            <div style={{
              ...
              {canNavigate && (
                <button
                  ...
                  title={t("msgview.editFromHereTitle")}
                  style={{
                    display: "flex", alignItems: "center", gap: 4,
                    padding: "3px 8px", height: 22,
                    background: "none", border: "none",
              ...
              {canFork && (
                <button
                  ...
                  disabled={forking}
                  title={forking ? t("msgview.creatingSession") : t("msgview.newSessionTitle")}
                  style={{
                    display: "flex", alignItems: "center", gap: 4,
                    padding: "3px 8px", height: 22,
                    background: "none", border: "none",
                ...
                >
                  ...
                  {forking ? t("msgview.creating") : t("chat.newSession")}
                </button>
              )}
            </div>
          )}
        ...
        {isStreaming && (() => {
          ...
          return (
            <>
              ...
              {est > 0 && (
                <span style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--text)" }} title={t("msgview.estimatedTokens")}>
...
function ThinkingBlock({ block, duration, sessionId, entryId, blockIndex }: {
  ...
  const toggle = async () => {
    ...
    if (!sessionId || !entryId) {
      setError(t("msgview.thinkingUnavailable"));
      return;
    }
  ...
  return (
    ...
    >
      ...
      {expanded && (
        ...
        >
          {loading ? t("general.loading") : error ?? (block.deferred ? content : block.thinking)}
        </div>
      )}
    </div>
  );
      ...
      {files.map((file, fileIndex) => (
        ...
        >
          {showFileHeaders && (
            ...
            >
              <SplitDiffHeader title={file.oldPath || t("msgview.before")} side="left" />
              <SplitDiffHeader title={file.newPath || t("msgview.after")} side="right" />
            </div>
          )}
...
function PairedResult({ text, isEmpty, isError }: {
  ...
  return (
    ...
    >
      ...
      >
        {isEmpty ? t("msgview.noOutput") : text}
      </pre>
    </div>
  );
}
      ...
      >
        ...
        <div style={{ padding: "11px 13px 12px" }}>
          <div style={{ color: "var(--text)", fontSize: 15, fontWeight: 700, lineHeight: 1.35 }}>
            {t("msgview.compactedTitle")}
          </div>
          <div style={{ marginTop: 3, marginBottom: 10, color: "var(--text)", fontSize: 14, lineHeight: 1.5 }}>
            {t("msgview.compactedBody")}
          </div>
          {parsedSummary.body ? (
            <MarkdownBody className="markdown-compaction-message">{parsedSummary.body}</MarkdownBody>
          ) : (
            <span style={{ color: "var(--text-dim)", fontSize: 12 }}>{t("msgview.noSummary")}</span>
          )}
...
function CompactionFileMetadata({ readFiles, modifiedFiles }: { readFiles: string[]; modifiedFiles: string[] }) {
  ...
  const parts = [];
  if (readFiles.length > 0) parts.push(t("msgview.filesRead", { count: readFiles.length }));
  if (modifiedFiles.length > 0) parts.push(t("msgview.filesModified", { count: modifiedFiles.length }));

  return (
    <details className="compaction-file-details">
      <summary>{t("msgview.fileContext", { parts: parts.join(", ") })}</summary>
      {modifiedFiles.length > 0 && <CompactionFileList title={t("msgview.modifiedFiles")} files={modifiedFiles} />}
      {readFiles.length > 0 && <CompactionFileList title={t("msgview.readFiles")} files={readFiles} />}
    </details>
  );
}
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        ...
        >
          ...
          {isHiddenDisplay && <span style={{ color: "var(--text-dim)", fontSize: 11 }}>{t("msgview.hiddenExtension")}</span>}
          {time && <span style={{ marginLeft: "auto", color: "var(--text-dim)", fontSize: 10 }}>{time}</span>}
        </div>
        ...
        {contentExpanded ? (
          <div style={{ padding: "6px 9px" }}>
            ...
            {text ? <MarkdownBody className="markdown-custom-message" cwd={cwd} onOpenFile={onOpenFile}>{text}</MarkdownBody> : <span style={{ color: "var(--text-dim)", fontSize: 12 }}>{t("msgview.noMessage")}</span>}
          </div>
        ) : (
          <button
          ...
          >
            {text ? previewText(text, t("msgview.showExtension")) : t("msgview.showExtension")}
          </button>
        )}
        ...
        >
          ...
          {(hasDetails || isHiddenDisplay) && (
            ...
            >
              {isHiddenDisplay
                ? (contentExpanded ? t("msgview.collapse") : t("msgview.expand"))
                : (detailsExpanded ? t("msgview.hideDetails") : t("msgview.showDetails"))}
            </button>
          )}
        </div>
...
function formatUsage(usage: {
  ...
  const parts = [];
  if (usage.input) parts.push(t("msgview.usageInput", { n: usage.input.toLocaleString() }));
  if (usage.output) parts.push(t("msgview.usageOutput", { n: usage.output.toLocaleString() }));
  if (usage.cacheRead) parts.push(t("msgview.usageCacheRead", { n: usage.cacheRead.toLocaleString() }));
  if (usage.cacheWrite) parts.push(t("msgview.usageCacheWrite", { n: usage.cacheWrite.toLocaleString() }));
  ...
  async function loadFullOutput() {
    ...
    try {
      ...
      if (d.success) {
        ...
        setFullError(d.error ?? t("msgview.loadFailed"));
      }
    } catch (e) {
      ...
      {message.truncated && fullOutputUrl && (
        <div style={{ padding: "4px 10px", fontSize: 11, marginTop: -1 }}>
          {showFullButton && (
            ...
            >
              {loadingFull ? t("general.loading") : t("msgview.viewFullOutput")}
            </button>
          )}
          <a
