...
async function fetchEntries(dirPath: string, t: Translate): Promise<FileNode[]> {
  ...
  if (!res.ok) {
    let message = t("explorer.loadFailed", { status: res.status });
    try {
...
const GIT_STATUS_LABEL_KEYS: Record<GitFileStatusKind, string> = {
  modified: "explorer.gitModified",
  added: "explorer.gitAdded",
  deleted: "explorer.gitDeleted",
  renamed: "explorer.gitRenamed",
  untracked: "explorer.gitUntracked",
  conflict: "explorer.gitConflict",
};
...
function GitStatusBadge({ status, t }: { status: GitFileStatus; t: Translate }) {
  return (
    <span
      title={t(GIT_STATUS_LABEL_KEYS[status.status])}
      aria-label={t(GIT_STATUS_LABEL_KEYS[status.status])}
      style={{
        width: 14,
        height: 14,
        flexShrink: 0,
...
function uploadFiles(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error(t("explorer.networkError")));
    xhr.onabort = () => reject(new Error(t("explorer.uploadCancelled")));
    xhr.onload = () => {
  ...
  return (
    <div>
      ...
      >
        ...
        {highlighted && (
          <span
            title={t("explorer.newlyUploaded")}
            aria-label={t("explorer.newlyUploaded")}
        ...
        {!hovered && containsGitChanges && (
          <span
            title={t("explorer.containsChanges")}
            aria-label={t("explorer.containsChanges")}
            style={{
              width: 14,
              height: 14,
              flexShrink: 0,
...
export const FileExplorer = forwardRef<FileExplorerHandle, Props>(function FileExplorer({
  ...
  const performUpload = useCallback(async (
    ...
    try {
      ...
      if (status < 200 || status >= 300) {
        throw new Error(data.error ?? t("explorer.uploadFailed", { status }));
      }
  ...
  const prepareUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? t("explorer.uploadCheckFailed", { status: res.status }));
  ...
  }, [cwd, performUpload, uploadBusy, t]);
  ...
  }, [cwd, refreshKey, treeRefreshKey, t]);
  ...
  return (
    <div style={{ minHeight: "100%" }}>
      ...
      {showUploadFeedback && (
        ...
        {uploadBusy && (
          <div role="status" aria-live="polite" aria-label={uploadPhase === "checking" ? t("explorer.checkingFiles") : t("explorer.uploadingProgress", { progress: uploadProgress })}>
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, #f59e0b 55%, var(--border))", borderRadius: 4, background: "color-mix(in srgb, #f59e0b 9%, var(--bg-panel))" }}>
            <div style={{ fontSize: 11, color: "var(--text)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
              {t("explorer.conflictDesc", { count: pendingConflict.conflicts.length, files: pendingConflict.conflicts.join(", ") })}
            </div>
            {pendingConflict.nonReplaceable.length > 0 && (
              <div style={{ marginTop: 3, fontSize: 10, color: "#f59e0b", lineHeight: 1.35, overflowWrap: "anywhere" }}>
                {t("explorer.cannotReplace", { files: pendingConflict.nonReplaceable.join(", ") })}
              </div>
            )}
        ...
        {uploadError && (
          <div role="alert" style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: 11, lineHeight: 1.35, color: "#f87171" }}>
            ...
            <DismissButton onClick={() => setUploadError(null)} title={t("explorer.dismissError")} />
          </div>
        )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 22, fontSize: 11 }}>
              <div style={{ minWidth: 0, flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
                {uploadSummary.uploaded.length > 0 && (
                  <span title={t("explorer.uploadedCount", { count: uploadSummary.uploaded.length })} aria-label={t("explorer.uploadedCount", { count: uploadSummary.uploaded.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "#22c55e" }}>
                ...
                {uploadSummary.skipped.length > 0 && (
                  <span title={t("explorer.skippedCount", { count: uploadSummary.skipped.length })} aria-label={t("explorer.skippedCount", { count: uploadSummary.skipped.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "var(--text-dim)" }}>
                ...
                {uploadSummary.errors.length > 0 && (
                  <span title={t("explorer.failedCount", { count: uploadSummary.errors.length })} aria-label={t("explorer.failedCount", { count: uploadSummary.errors.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "#f87171" }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              ...
              {uploadSummary.uploaded.length > 0 && onAtMentions && (
                <button
                  ...
                  title={t("explorer.addUploadedToChat")}
                  aria-label={t("explorer.addUploadedToChat")}
              ...
              <DismissButton onClick={() => setUploadSummary(null)} title={t("explorer.dismissResults")} />
            </div>
      ...
      {(changesCollapsed || gitFiles.length === 0) && (
        <div style={{ padding: "2px 4px" }}>
          {loading ? (
            <div style={{ padding: "8px 12px", fontSize: 11, color: "var(--text-dim)" }}>{t("general.loading")}</div>
          ) : error ? (
