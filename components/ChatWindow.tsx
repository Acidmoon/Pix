...
function phaseLabel(phase: AgentPhase, t: TFunc): string {
  if (phase?.kind === "running_tools") {
    const names = phase.tools.map((tool) => tool.name);
    if (names.length === 0) return t("chatwin.runningTool");
    if (names.length === 1) return t("chatwin.runningToolNamed", { name: names[0] });
    if (names.length <= 3) return t("chatwin.runningTools", { names: names.join(", ") });
    return t("chatwin.runningToolsMore", { names: names.slice(0, 2).join(", "), more: names.length - 2 });
  }
  if (phase?.kind === "waiting_model") return t("chatwin.waitingModel");
  if (phase?.kind === "running_command") return t("chatwin.runningCommand");
...
function ProcessDetailsGroup({ messageCount, toolCallCount, children }: { messageCount: number; toolCallCount: number; children: ReactNode }) {
  ...
  const parts = [t("chatwin.processDetails"), t("chatwin.messages", { count: messageCount })];
  if (toolCallCount > 0) parts.push(t("chatwin.toolCalls", { count: toolCallCount }));
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <button
        ...
        title={expanded ? t("chatwin.collapseProcessDetails") : t("chatwin.expandProcessDetails")}
      >
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto pt-4 [scrollbar-width:none]">
          <div style={{ padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
                ...
                <>
                  {hasMore && (
                    <div ref={sentinelRef} className="py-3 text-center text-xs text-text-muted">
                      {t("chatwin.scrollUpToLoad", { count: startIndex })}
                    </div>
                  )}
            ...
            {bashRunning && !pendingBash && (
              <div className="py-2 text-[13px] text-text-muted">
                <span className="animate-[pulse_1.5s_infinite]">{t("chatwin.runningCommand")}</span>
              </div>
            )}
    ...
    >
      ...
      >
        <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--border)" }}>
          ...
          <div style={{ marginTop: 3, color: "var(--text-dim)", fontSize: 11, fontFamily: "var(--font-mono)" }}>{t("chatwin.extensionRequest")}</div>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        <textarea
          ref={inputRef}
          aria-label={t("chatwin.extensionTerminalInput")}
          autoCapitalize="off"
          autoComplete="off"
          autoCorrect="off"
          spellCheck={false}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "10px 12px", borderBottom: "1px solid var(--border)" }}>
          <div style={{ color: "var(--text)", fontSize: 13, fontWeight: 650 }}>{t("chatwin.extensionPanel")}</div>
          <button
